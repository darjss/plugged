...
function key() {
  return env.POSTHOG_KEY ?? env.PUBLIC_POSTHOG_KEY;
}
...
export function posthogConfigured() {
  const apiKey = key();
  return Boolean(apiKey && apiKey !== "phc_XXX");
}
...
export async function captureServerEvent(
  ...
  const apiKey = key();
  if (!apiKey || apiKey === "phc_XXX") return;
...
async function posthogQuery<T>(query: string): Promise<T | null> {
  ...
  const apiKey = key();
  if (!projectId || !apiKey || apiKey === "phc_XXX") return null;
