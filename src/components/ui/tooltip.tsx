...
const Tooltip = (props: TooltipPrimitive.TooltipRootProps) => {
  const mergedProps = mergeProps(
    { openDelay: 0, placement: "top" } as TooltipPrimitive.TooltipRootProps,
    props,
  );
  ...
  return (
    <TooltipPrimitive.Portal>
      <TooltipPrimitive.Content
        ...
        class={cn(
          "z-50 max-w-xs bg-ink text-newsprint border-2 border-ink px-3 py-1.5 text-xs font-black uppercase tracking-wide shadow-hard-sm",
          local.class,
        )}
        {...(others as any)}
      >
